...
switch ($action) {
    ...
    case 'submit_registration':{
        ...
        if ($email == NULL || $fname == NULL || $lname == NULL || $birthday == NULL || $password == NULL) {
            $error = 'Email, FirstName, LastName, Birthday and Password not included';
            include('errors/error.php');
        } else {
            $user = AccountsDB::submit_registration($email, $fname, $lname, $birthday, $password,);
            $userId = $user->getId();
            if ($userId == false) {
                header('Location: index.php?action=display_registration');
            } else {
